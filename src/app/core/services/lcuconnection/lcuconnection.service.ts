...
export class LCUConnectionService {
  ...
  private async makeRequest(method: string, body: Record<string, unknown>, endPoint: string, getFull: boolean): Promise<any> {
    if (!this.connector || !this.connector.connector) {
      return "League Client is not running or not connected! Please open League of Legends first.";
    }
    const options = JSON.parse(JSON.stringify(this.connector.connector));
    const cleanEndpoint = (endPoint && endPoint.endsWith('/') && endPoint.length > 1) ? endPoint.slice(0, -1) : endPoint;
    options.url += cleanEndpoint;
    ...
    return await this.electronService.request(options)
      ...
      .catch(err => {
        return err && err.error ? (typeof err.error === 'string' ? err.error : JSON.stringify(err.error)) : (err && err.message ? err.message : 'Error');
      });
  }
